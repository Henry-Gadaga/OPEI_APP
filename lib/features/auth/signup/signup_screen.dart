...
class _SignupScreenState extends ConsumerState<SignupScreen>
  ...
  String? _validateEmail(String v) {
    final t = v.trim();
    if (t.isEmpty) return _l10n.emailRequiredError;
    if (!RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$').hasMatch(t)) {
      return _l10n.emailInvalidError;
    }
    return null;
  }
  ...
  String? _validatePin(String v) {
    ...
    if (!RegExp(r'^\d{6}$').hasMatch(v)) return _l10n.pinInvalidError;
    return null;
  }
  ...
  void _next(bool isLoading) {
    ...
    if (_step == 0) {
      ...
      final fullPhone = OpeiPhoneField.e164(
        _phoneIso,
      ...
      if (next is SignupSuccess) {
        ...
        if (next.fieldErrors.isNotEmpty) {
          setState(() {
            ...
            _phoneError =
                next.fieldErrors['phone'] ??
                next.fieldErrors['phoneNumber'] ??
                _phoneError;
            _pinError =
                next.fieldErrors['pin'] ??
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      AnimatedSwitcher(
                        ...
                        child: Text(
                          <String>[
                            l10n.signupSubtitleEmail,
                            l10n.signupSubtitlePhone,
                            l10n.signupSubtitlePin,
                          ][_step],
                          key: ValueKey(_step),
                          style: TextStyle(
                            fontFamily: kPrimaryFontFamily,
    ...
    return Column(
      ...
      children: [
        ...
        if (_step == 0) ...[
          ...
          _FieldLabel(text: l10n.phoneNumberLabel),
          const SizedBox(height: 8),
          OpeiPhoneField(
            label: '',
            enabled: !isLoading,
            ...
            onIsoChanged: (iso) => setState(() {
              _phoneIso = iso;
              _phoneError = null;
            }),
            onLocalNumberChanged: (v) {
              if (_localPhoneController.text != v) {
                _localPhoneController.value = _localPhoneController.value
                    .copyWith(
                      text: v,
          ...
          _FieldLabel(text: l10n.pinLabel),
          const SizedBox(height: 8),
          OpeiTextField(
        ...
        // ── Primary CTA ────────────────────────────────────────────────
        OpeiPrimaryButton(
          label: _step < 2 ? l10n.continueCta : l10n.signupCreateAccountCta,
          loading: isLoading,
